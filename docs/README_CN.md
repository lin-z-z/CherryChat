...
`v1.2.1` 在 `v1.2.0` 的无状态逐请求访问码校验、`v1.1.0` 首次建立的稳定契约之上，
修复了一个 Hosted 认证状态缺陷。本目录收录不适合
全部堆放在主 README 中的技术说明和边界。
...
- [发布与版本政策](./RELEASES_CN.md) —— 稳定版本兼容性、质量门、手动发布自动化和
  公开 Tag 不可移动原则。
- [变更记录](../CHANGELOG_CN.md) —— 已跟踪版本、主要能力、已知限制、备份建议和
  部署边界。
- [开源许可证与归属（英文）](../LICENSES.md) —— 主要第三方依赖和许可证信息。

## 项目成熟度

`v1.2.1` 延续 `v1.2.0` 已纳入文档的稳定产品契约（含无状态 Hosted access 校验），
并受到仓库质量门保护；它不承诺长期支持、托管服务可用性或企业服务。
