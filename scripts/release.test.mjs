...
test("reads matching package and bilingual changelog metadata", async () => {
  ...
  await writeJson("package.json", { version: "1.2.1" });
  await writeJson("package-lock.json", {
    version: "1.2.1",
    packages: { "": { version: "1.2.1" } },
  });
  const changelog =
    "# Changelog\n\n## [1.2.1] - 2026-08-31\n\n### Summary\n\nHosted auth epoch ownership.\n";
  ...
  assert.equal(metadata.version, "1.2.1");
  assert.equal(metadata.tagName, "v1.2.1");
  assert.equal(metadata.title, "CherryChat v1.2.1");
