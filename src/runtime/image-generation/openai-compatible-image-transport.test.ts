...
describe("OpenAICompatibleImageTransport", () => {
  ...
  it("preserves reference order in repeated multipart image fields", async () => {
    ...
    await createTransport(fetchMock).generate({
    ...
    expect(form.get("output_compression")).toBe("76");
  });
  ...
  it("omits an empty Hosted profile id for legacy multipart requests", async () => {
    const { calls, fetchMock } = createFetchMock();
    const transport = new OpenAICompatibleImageTransport({
      endpoint: {
        mode: "hosted",
    ...
    await transport.generate(request([attachment("legacy", "image/png")]));

    const form = calls[0]?.[1]?.body as FormData;
    expect(form.has("profileId")).toBe(false);
  });
