...
**Final pass (before Phase 3.4 commit)**: the last 2.2 of a task must run full-scope for the task's complete impact surface, not just on the latest implement chunk. Full-scope does not unconditionally mean the repository's Push/PR/release matrix. List all affected packages with `python ./.trellis/scripts/get_context.py --mode packages`, then load each package's spec index Quality Check section. Escalate shared schemas, security boundaries, migrations, dependencies/lockfiles, build/test infrastructure, and other high-fan-out contracts to the full repository gate. This catches cross-layer / multi-package issues while keeping unrelated release checks out of the normal local loop.
