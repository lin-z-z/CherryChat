...
- Network search is available only when global settings, the active
  ...
  resolves only authenticated site search; BYOK resolves only a personal Key/URL.
  A conversation
...
| No saved search preference and Hosted Tavily is available | Initialize global search permission on without persisting it |
| Saved search preference is off and Tavily is available | Restore off; source availability must not override user intent |
...
- Good: switching from access-code mode to Custom API keeps the personal Tavily
  draft but requires that personal source; it never spends the deployment key.
- Good: the user saves Custom API and turns global search off; reload restores
  both choices even though Hosted model and Tavily services remain available.
...
- Repository/Playwright: source-aware defaults apply only without records;
  explicit Custom API and search opt-out choices survive reload on a Hosted
  deployment.
